...
export function calculatePilotPlanFact(
  ...
  const qualityText =
    actualQuality >= planQuality
      ? `Качество генерации (${Math.round(actualQuality * 100)}%) достигло целевого значения (${Math.round(planQuality * 100)}%).`
      : actualQuality >= minQuality
      ? `Качество генерации (${Math.round(actualQuality * 100)}%) ниже целевого значения (${Math.round(planQuality * 100)}%), но выше минимально допустимого порога (${Math.round(minQuality * 100)}%).`
      : `Качество генерации (${Math.round(actualQuality * 100)}%) ниже минимально допустимого порога (${Math.round(minQuality * 100)}%).`;

  if (actualEffect > 0 && actualQuality >= minQuality && forecastError >= -25) {
    return {
      forecastError,
      deltaRub,
  ...
  return {
    ...
    reasons: [
      ...
      qualityText,
      "Дальнейшие инвестиции не рекомендуются до пересмотра процесса.",
    ],
  };
}
